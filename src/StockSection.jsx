...
import ControlesSection from "./ControlesSection.jsx";
...
const Ico = {
  ...
  clip:    IcoSvg(<><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1" ry="1"/><path d="M9 12l2 2 4-4"/></>,18),
...
const TABS = [
  ...
  {id:"controles",  label:"Contrôles",  ico:Ico.clip},
];
...
export default function StockSection({
  ...
  const printBl = (bl) => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>BL ${bl.num}</title><style>body{font-family:-apple-system,sans-serif;padding:40px 40px 80px;color:#0f172a;max-width:820px;margin:0 auto}h1{font-size:22px;font-weight:900;margin:0 0 4px}.sub{color:#64748b;font-size:13px;margin-bottom:28px}table{width:100%;border-collapse:collapse;margin-top:12px}th{background:#f4f7fa;padding:8px 12px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#475569;font-weight:700}td{padding:10px 12px;border-bottom:1px solid #e8edf3;font-size:13px}.tot{margin-top:16px;text-align:right;font-size:15px;font-weight:900;color:#FC7701}.hd{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px}.brand{font-size:20px;font-weight:900;color:#FC7701}.footer{position:fixed;bottom:24px;right:36px;opacity:.75}@media print{body{padding:20px 20px 80px}.footer{position:fixed;bottom:24px;right:36px}}</style></head><body><div class="hd"><div><h1>Bon de réception</h1><div class="sub">${bl.num} · ${bl.dateLabel||fmtDate(bl.date)||'—'}${bl.createdBy?' · Saisi par '+bl.createdBy:''}</div></div><div class="brand">GTK STOCK</div></div><table><thead><tr><th>Désignation</th><th>Type</th><th style="text-align:center">Qté</th><th style="text-align:right">Prix unit.</th><th style="text-align:right">Total</th></tr></thead><tbody>${rows}</tbody></table>${total>0?`<div class="tot">Total : ${total.toFixed(2).replace('.',',')} €</div>`:''}<div class="footer"><img src="${logo}" style="height:44px;width:auto"/></div><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };
  ...
  const printSorties = (sorties, mode="detail") => {
    ...
    const w=window.open('','_blank');
    w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Bon de sortie</title><style>${CSS}</style></head><body><div class="hd"><div><h1>Bon de sortie matériel</h1><div class="sub">${periodeStr?periodeStr+' · ':''}${modeLabel} · Édité le ${dateStr}</div></div><div class="brand">GTK STOCK</div></div>${sections}${totalGlobal>0?`<div class="tot-global">Total général : ${totalGlobal.toFixed(2).replace('.',',')} €</div>`:''}<div class="footer"><img src="${logo}" style="height:44px;width:auto"/></div><script>window.onload=()=>window.print()<\/script></body></html>`);
    w.document.close();
  };
  ...
  const renderEntrees = () => (
    <div>
      {blMode==="liste" ? (<>
        ...
        {/* ── Panier (sticky TOP) ── */}
        {blLignes.length>0&&(
          <div ref={blCartRef} style={{position:"sticky",top:0,zIndex:10,marginBottom:12}}>
                    ...
                    {keys.map(cat=>(
                      <div key={cat}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                          ...
                          <span style={{fontSize:10,color:T5}}>· {groups[cat].length}</span>
                                ...
                                <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
                                  ...
                                  <div style={{flex:1,minWidth:0}}>
                                    ...
                                    <div style={{display:"flex",gap:5,marginTop:2,alignItems:"center",flexWrap:"wrap"}}>
                                      ...
                                      {c.prix>0&&<span style={{fontSize:10,fontFamily:FM,color:T4}}>{f(c.prix)}</span>}
                                    </div>
                                  </div>
                                  {inCart ? (
                                    <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                                      <button onClick={()=>cartDelta(inCart.id,-1)} style={{width:30,height:30,borderRadius:7,background:C2,border:"none",cursor:"pointer",fontWeight:900,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",color:T2}}>−</button>
                                      <input type="number" value={inCart.qty} min="1" onChange={e=>cartSetQty(inCart.id,e.target.value)} onClick={e=>e.target.select()}
                                        style={{width:38,textAlign:"center",fontFamily:FM,fontWeight:800,fontSize:14,border:`1.5px solid ${O}`,borderRadius:6,padding:"4px 2px",color:OD,background:C1,outline:"none"}}/>
    ...
    <div>
      ...
      {isAdmin&&showForm&&(
        <div style={{marginBottom:16}}>
          ...
          {/* ── Panier sorties (sticky TOP) ── */}
          {sortCart.length>0&&(
            <div ref={sortCartRef} style={{position:"sticky",top:0,zIndex:10,marginBottom:12}}>
            <Card style={{boxShadow:"0 4px 20px rgba(0,0,0,.12)",border:`1.5px solid ${O}30`}}>
                              ...
                              return (
                                <div key={a.id||a.nom} style={{background:inCart?OL:C1,border:`1.5px solid ${inCart?O:C2}`,borderRadius:10,fontFamily:FF,transition:"all .12s",opacity:remaining<=0&&!inCart?.45:1}}>
                                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px"}}>
                                    ...
                                    {inCart ? (
                                      <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                                        <button onClick={()=>sortCartDelta(inCart.id,-1)} style={{width:30,height:30,borderRadius:7,background:C2,border:"none",cursor:"pointer",fontWeight:900,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",color:T2}}>−</button>
                                        <input type="number" value={inCart.qty} min="1" max={inCart.stock} onChange={e=>sortCartSetQty(inCart.id,e.target.value)} onClick={e=>e.target.select()}
                                          style={{width:38,textAlign:"center",fontFamily:FM,fontWeight:800,fontSize:14,border:`1.5px solid ${O}`,borderRadius:6,padding:"4px 2px",color:OD,background:C1,outline:"none"}}/>
  ...
  const renderTechs = () => {
    ...
    const pillBase = {border:"none",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",transition:"all .12s"};
    return (
    <div>
      ...
      {availMonths.length>0&&(
        <div style={{overflowX:"auto",marginBottom:14,paddingBottom:2,WebkitOverflowScrolling:"touch"}}>
          <div style={{display:"flex",gap:5,minWidth:"max-content",paddingRight:4}}>
            <button onClick={()=>setTechsFilter("")}
              style={{...pillBase,background:techsFilter===""?T2:C2,color:techsFilter===""?"#fff":T4}}>
              Tout
            </button>
            {availMonths.map(ym=>(
              <button key={ym} onClick={()=>setTechsFilter(ym)}
                style={{...pillBase,background:techsFilter===ym?O:C2,color:techsFilter===ym?"#fff":T4}}>
                {fmtYM(ym)}
              </button>
            ))}
          </div>
      ...
      {techs.length===0
        ...
        : <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[...techs].sort((a,b)=>techFullName(a).localeCompare(techFullName(b),"fr")).map(t=>{
              // Filtre par mois si sélectionné, sinon tout
  ...
  // RENDER PRINCIPAL
  // ════════════════════════════════════════════════════════
  const LABELS = {inventaire:"Inventaire",entrees:"Entrées",sorties:"Sorties",catalogue:"Catalogue",stats:"Statistiques",techs:"Équipe",controles:"Contrôles"};
  const SUBTITLES = {
    ...
    controles:  "Véhicule · EPI · Outillage · Matériel",
  };
    ...
    <div style={{display:"flex",fontFamily:FF,height:"100%"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:C3,borderRadius:isMob?0:"0 12px 12px 0",minWidth:0}}>
        ...
        <div ref={scrollRef} style={{flex:1,overflowY:"auto",padding:"16px",paddingBottom:isMob?80:16}}>
          ...
          {tab==="controles" &&<ControlesSection techs={techs} currentUser={currentUser} onToast={onToast} isAdmin={isAdmin}/>}
        </div>
      </div>
